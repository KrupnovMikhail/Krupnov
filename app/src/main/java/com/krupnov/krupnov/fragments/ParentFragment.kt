...
open class ParentFragment : Fragment() {
    ...
    protected fun back() {
        ...
        try {
            ...
            val myToast = Toast.makeText(
                requireContext(),
                "Что-то пошло не так. Нажмите стрелку еще раз",
                Toast.LENGTH_LONG
            )
            myToast.show()
    ...
    protected fun currGif() {
        if (index < 0) {
            ...
            mImageView.setImageResource(R.drawable.ic_baseline_photo_24)
        }
    }
        ...
        try {
            ...
            while (jSONcounter >= currObj.length()) {
                ...
                if (flag || response.getException() != null) {
                    val myToast = Toast.makeText(
                        requireContext(),
                        "Не удалось подключиться к сайту, проверьте подключение к интернету и попробуйте ещё раз",
                        Toast.LENGTH_LONG
                    )
                    myToast.show()
                    index++
